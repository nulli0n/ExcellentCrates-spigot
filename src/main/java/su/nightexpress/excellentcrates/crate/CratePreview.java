...
public class CratePreview extends AbstractMenuAuto<ExcellentCrates, ICrateReward> {
	...
	private boolean hideDrainedRewards;
	...
	protected ItemStack getObjectStack(@NotNull Player player, @NotNull ICrateReward reward) {
		...
		if (rewardLimit == null || rewardLimit.isDrained(reward) || !reward.isWinLimitedAmount()) lore.remove(PLACEHOLDER_WIN_LIMIT_AMOUNT);
		if (rewardLimit == null || rewardLimit.isDrained(reward) || rewardLimit.isExpired()) lore.remove(PLACEHOLDER_WIN_LIMIT_COOLDOWN);
