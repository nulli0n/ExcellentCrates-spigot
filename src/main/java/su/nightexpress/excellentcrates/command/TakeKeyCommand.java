    ...
    public void onExecute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
        ...
        if (pName.equalsIgnoreCase(Constants.MASK_ANY)) {
            for (Player player : plugin.getServer().getOnlinePlayers()) {
                plugin.getKeyManager().takeKey(player, crateKey, amount);

                plugin.lang().Command_TakeKey_Notify
            ...
            if (!plugin.getKeyManager().takeKey(pName, crateKey, amount)) {
                this.plugin.lang().Command_TakeKey_Error.send(sender);
                return;
            }
        }

        plugin.lang().Command_TakeKey_Done
